...
def load_dataset_text(data_path, char_to_idx):
    """Load dataset text for random context sampling"""
    with open(data_path, "r", encoding="utf-8") as f:
        text = f.read()
    return encode_text(text, char_to_idx)
...
def generate_samples(model, idx_to_char, num_samples=5, temperature=1.0, dataset_tokens=None):
    ...
    for i in range(num_samples):
        with torch.no_grad():
            ...
            text = decode_tokens(tokens[0], idx_to_char)
...
def generate_continuous_blocks(
    model, idx_to_char, num_blocks=30, temperature=1.0, dataset_tokens=None
):
    """
    ...
    Args:
        ...
        dataset_tokens: Dataset tokens for initial context
    """
    device = model.get_device()
    context_len = model.config.context_len
...
def main():
    ...
    if model.config.context_len > 0:
        ...
        if char_to_idx is None:
            # Old checkpoint format - build vocab from dataset
            data_path = "data/tiny_shakespeare.txt"
            with open(data_path, "r", encoding="utf-8") as f:
                text = f.read()
            char_to_idx, idx_to_char, _ = build_vocab(text)
        dataset_tokens = load_dataset_text("data/tiny_shakespeare.txt", char_to_idx)
